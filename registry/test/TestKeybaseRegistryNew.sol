...
contract TestKeybaseRegistryNew {
  ...
  function testDoubleRegisterNewWithUnregister() {
    KeybaseRegistry meta = new KeybaseRegistry();
    address expected = 0x0;
    Assert.equal(meta.getAddress("ice09"), expected, "Address should be empty.");
    ...
    KeybaseRegistry(address(throwProxy)).changeOwner(address(this));
    throwProxy.execute();
    
    meta.unregister("ice09");
    expected = 0x0;
    Assert.equal(meta.getAddress("ice09"), expected, "Address should be empty.");
